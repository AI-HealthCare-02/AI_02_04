...
def get_diet_history(
    ...
    logs = diet_service.get_diet_history(
        db, current_user["user_id"], start_date, end_date, limit
    )
    return {
        "success": True,
        "data": [
...
def update_diet_manual(
    ...
    result = diet_service.update_diet_manual(
        db, current_user["user_id"], diet_id, data.food_name
    )
    return {
        "success": True,
        "data": {
...
async def analyze_diet(
    ...
    try:
        image_url = await upload_diet_image(tmp_path, current_user["user_id"])
        ...
        return {"success": True, "data": api_res}
    finally:
        os.unlink(tmp_path)
