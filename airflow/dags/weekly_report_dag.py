...
def get_all_users():
    ...
    with engine.connect() as conn:
        result =  conn.execute(text("SELECT id, age, gender, bmi, exercise_frequency, smoking, drinking, family_history FROM users"))
...
with DAG(
    ...
    task2 = PythonOperator(
        task_id = "repredict_users",
        python_callable = repredict_users
    )
